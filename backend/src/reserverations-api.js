...
export function setupReservationApi(app) {
  app.get("/api/reservations", (req, res) => {
    ...
    if (!allowedOrderBy.includes(orderBy)) {
      return res.status(400).json({"error": `Invalid 'orderBy' search param '${orderBy}' Allowed values: '${allowedOrderBy}'`})
    }
    return res.status(200).json(sortReservations(reservations, orderBy));
  })

  app.post("/api/reservations", (req, res) => {
    const {foodTruck, customerName,timeRange, expectedGuests, specialRequests } = req.body || {};
    ...
    if (!foodtrucks.find(f => f.name === foodTruck)) {
      errors.push({error: "'foodTruck' must be valid foodtruck, one of: " +
    ...
    if (!timeRange || typeof timeRange !== "object") {
      ...
      if (!dayjs(timeRange.start).isValid()) {
        errors.push({error: "'timeRange.start' must be a valid ISO date time"})
      }
    ...
    const newReservation = {
      ...
      foodTruck, customerName,timeRange, expectedGuests, specialRequests,
      status: "Requested"
    }
  ...
  app.get("/api/reservations/:reservationId", (req, res) => {
    ...
    const reservation = reservations.find(d => d.id === req.params.reservationId);
    if (!reservation) {
      return res.status(404).json({error: `Reservation not found with id '${req.params.reservationId}'`});
    }
  ...
  app.put("/api/reservations/:reservationId/status", (req, res) => {
    ...
    if (newStatus !== "Rejected" && newStatus !== "Confirmed") {
      return res.status(400).json({error: `Invalid Status ${newStatus}. Please use 'Rejected' or 'Confirmed'`});
    }
